...
/**
 * CPSC 424, Fall 2015, Lab 6: Light and Material in OpenGL 1.1. This program
 * shows a square "stage" with a variety of objects arranged on it. The objects
 * use several shapes and materials and include a wireframe object that is drawn
 * with lighting turned off. The user can rotate the stage about the y-axis by
...
public class Lab6 extends GLJPanel implements GLEventListener {
	...
	/**
	 * Constructor for class Lab6, sets the preferred size and configures event
	 * listeners
	 */
	public Lab6() {
	...
	/**
	 * One of the rows of this array corresponds to a set of material properties.
	 * Items 0 to 4 in a row specify an ambient color; items 4 through 7, a diffuse
	 * color; items 8 through 11, a specular color; and item 12, a specular exponent
	 * (shininess value). The data is adapted from the table on the page
	...
	public void display(GLAutoDrawable drawable) {
		...
		gl2.glMaterialfv(GL2.GL_FRONT, GL_AMBIENT_AND_DIFFUSE, new float[] { 0.0215f, 0.1745f, 0.0215f }, 0); //materialy
		myPyramid(gl2, 10); //inicjalizacja piramidy

	} // end display()

	public void Element(GL2 gl2, float a, float b, float c, float d) {
		gl2.glPushMatrix();
		gl2.glBegin(GL_TRIANGLE_FAN);
		gl2.glNormal3f(a, 1, b);
		gl2.glVertex3f(a, 0, b);
		gl2.glNormal3f(c, 1, d);
		gl2.glVertex3f(c, 0, d);
		gl2.glVertex3f(0, 2, 0);
		gl2.glEnd();
		gl2.glPopMatrix();
	}

	public void myPyramid(GL2 gl2, int N) {    //piramidka

		float[] XPoint = new float[N];
		float[] YPoint = new float[N];
		XPoint[0] = (float) (2 * Math.cos((2.0 * Math.PI * 0) / N));
		YPoint[0] = (float) (2 * Math.sin((2.0 * Math.PI * 0) / N));
		...
		for (int i = 1; i < N; i++) {
			...
			XPoint[i] = (float) (2 * Math.cos((2.0 * Math.PI * i) / N));
			YPoint[i] = (float) (2 * Math.sin((2.0 * Math.PI * i) / N));
			Element(gl2, XPoint[i - 1], YPoint[i - 1], XPoint[i], YPoint[i]);

		}
		Element(gl2, XPoint[N - 1], YPoint[N - 1], XPoint[0], YPoint[0]);
	...
	public void init(GLAutoDrawable drawable) {
		...
		gl2.glLightfv(GL_LIGHT0, GL_POSITION, new float[] { 5, 5, 5, 0 }, 0);
		gl2.glLightfv(GL_LIGHT0, GL_SPECULAR, new float[] { rand.nextFloat(), rand.nextFloat(), rand.nextFloat(), 1 },
				0);
		gl2.glLightfv(GL_LIGHT0, GL_DIFFUSE, new float[] { 1f, 0.5f, 0f, 1 }, 0);
	}
