...
public class Lab5 extends GLJPanel implements GLEventListener, KeyListener{
    ...
    public void Piramida(GL2 gl2,double size){
    	//zewnetrze piramidy, mozna zmienic kolorki
        gl2.glBegin(GL2.GL_TRIANGLE_FAN);
        int n = 3;
        gl2.glVertex3f(0,3, 0);    //srodkowy parametr to wysokosc piramidy
        for ( int i = 0; i<n;i++)
        {
            gl2.glColor3f(0,1,0.5f);
            gl2.glVertex3f((float) (Math.cos(i*2*Math.PI/n)*size), -3, (float) (Math.sin(i*2*Math.PI/n)*size));
            gl2.glColor3f(0,1,0.5f);
            gl2.glVertex3f((float) (Math.cos((i+1)*2*Math.PI/n)*size), -3, (float) (Math.sin((i+1)*2*Math.PI/n)*size));
        }
        gl2.glEnd();
        //srodek piramidy, mozna zmienic kolorki
        gl2.glBegin(gl2.GL_POLYGON);
        gl2.glVertex3f(0,0,0);
        for (int i=0; i<n;i++)
        {
            gl2.glColor3f(0,1,0.1f);
            gl2.glVertex3f((float) (Math.cos(i*2*Math.PI/n)*size), -3, (float) (Math.sin(i*2*Math.PI/n)*size));
            gl2.glColor3f(0,1,0.1f);
            gl2.glVertex3f((float) (Math.cos((i+1)*2*Math.PI/n)*size), -3, (float) (Math.sin((i+1)*2*Math.PI/n)*size));
        }
    ...
    public void Korkociag(GL2 gl2,int radius){
        gl2.glClear(GL2.GL_COLOR_BUFFER_BIT);

        int numVertices = 4000;  //jak ladny bedzie obrazek, ilosc wierzcholkow
        
        double angleIncrement = 2 * Math.PI / numVertices; // ilosc spiral
        float height=-2, x=0, z=0;
        double angle = 0;

        gl2.glBegin(GL2.GL_POLYGON);
        for (int i = 1; i <= numVertices; i++) {
            angle = i * angleIncrement * Math.PI;
            gl2.glColor3f(0,1,0.5f);			//kolor tego korkociagu, mozna zrobic np x*x z*z
